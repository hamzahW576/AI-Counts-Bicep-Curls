...
class App:
    ...
    def __intit__(self):
        self.window = tk.Tk()
        self.window.title = "Hamzah's STRONG Biceps Counter"
...
def init_gui (self):
    ...
    self.btn_train = tk.Button (self.window, text = "Train Model", width = 50, command = lambda: self.model.tain_model(self.counters))
...
def update(self):
    ...
    if ret:
        ...
        self.canavs.create_image(0, 0, image = self.photo, anchor = tk.NW)
...
def save_for_class(self, class_num):
    ...
    cv2.inwrite(f"{class_num}/frame{self.counters[class_num -1]}.jpg", cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY))
